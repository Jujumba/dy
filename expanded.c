...
# 138 "/usr/include/bits/mman-linux.h" 3 4
# 1 "/usr/include/bits/mman-shared.h" 1 3 4
# 139 "/usr/include/bits/mman-linux.h" 2 3 4
...
# 146 "/usr/include/sys/mman.h" 3 4
...
# 153 "/usr/include/sys/mman.h" 2 3 4
...
# 23 "src/arena.h" 3 4
   ((void) sizeof ((
# 23 "src/arena.h"
   ptr && "Failed to `mmap` 2 GibiBytes bro 😭"
# 23 "src/arena.h" 3 4
...
# 23 "src/arena.h"
   "ptr && \"Failed to `mmap` 2 GibiBytes bro 😭\""
# 23 "src/arena.h" 3 4
   , "src/arena.h", 23, __extension__ __PRETTY_FUNCTION__); }))
# 23 "src/arena.h"
                                                         ;
...
void* ArenaAlloc(Arena *this, u32 size) {
    if (this->allocated + size >= this->bound) 
# 29 "src/arena.h" 3 4
                                              ((void) sizeof ((
# 29 "src/arena.h"
                                              false && "Arena 2GiB limit exceeded. How?"
# 29 "src/arena.h" 3 4
                                              ) ? 1 : 0), __extension__ ({ if (
# 29 "src/arena.h"
                                              false && "Arena 2GiB limit exceeded. How?"
...
# 29 "src/arena.h"
                                              "false && \"Arena 2GiB limit exceeded. How?\""
# 29 "src/arena.h" 3 4
                                              , "src/arena.h", 29, __extension__ __PRETTY_FUNCTION__); }))
# 29 "src/arena.h"
                                                                                                ;
...
# 46 "src/arena.h" 3 4
   ((void) sizeof ((
# 46 "src/arena.h"
   status == 0 && "failed to unmap arena's buffer"
# 46 "src/arena.h" 3 4
   ) ? 1 : 0), __extension__ ({ if (
# 46 "src/arena.h"
   status == 0 && "failed to unmap arena's buffer"
...
# 46 "src/arena.h"
   "status == 0 && \"failed to unmap arena's buffer\""
# 46 "src/arena.h" 3 4
   , "src/arena.h", 46, __extension__ __PRETTY_FUNCTION__); }))
# 46 "src/arena.h"
                                                          ;
}
# 4 "test.c" 2
...
# 439 "/usr/include/bits/mathcalls.h" 3 4
...
# 489 "/usr/include/bits/mathcalls.h" 3 4
...
int main() {
    ...
    Numbers nums = {0};
    do { do { u32 remaining = (&nums)->header.cap - (&nums)->header.len; if (remaining >= 1) break; float additional_f = (float) 1; u32 additional_cap = (u32) powf(64.0f, ceil(additional_f / 64.0f)); u32 new_cap = (&nums)->header.cap + additional_cap; void* new_buffer = ArenaAlloc(&arena, new_cap * sizeof(* (&nums)->buffer)); if ((&nums)->header.len != 0 && (&nums)->buffer) { memcpy(new_buffer, (&nums)->buffer, (&nums)->header.len * sizeof(* (&nums)->buffer)); } (&nums)->buffer = new_buffer; (&nums)->header.cap = new_cap; } while (0);; (&nums)->buffer[(&nums)->header.len - 1] = 10; (&nums)->header.len += 1; } while (0);;
    printf("%d\n", nums.buffer);
}
